...
BLASTER / HYPERBLASTER (modded just blaster)
...
void Weapon_Blaster_Fire (edict_t *ent) // modded 4/4/17 (shoot quintuple bullets)
{
	int		damage;
	vec3_t tempvec;
	...
	//makes a bullet shoot from 8 units to the right
	VectorSet(tempvec, 0, 5, 0);
	VectorAdd(tempvec, vec3_origin, tempvec);
	Blaster_Fire(ent, tempvec, damage, false, EF_BLASTER);
	//makes a bullet shoot from 8 units to the left
	VectorSet(tempvec, 0, -5, 0);
	VectorAdd(tempvec, vec3_origin, tempvec);
	Blaster_Fire(ent, tempvec, damage, false, EF_BLASTER);
	//makes a bullet shoot from 8 units to the top
	VectorSet(tempvec, 0, 0, 5); //(x,y,z) z = top
	VectorAdd(tempvec, vec3_origin, tempvec);
	Blaster_Fire(ent, tempvec, damage, false, EF_BLASTER);
	//makes a bullet shoot from 8 units to the bottom
	VectorSet(tempvec,0 , 0, -5); 
	VectorAdd(tempvec, vec3_origin, tempvec);
	Blaster_Fire(ent, tempvec, damage, false, EF_BLASTER);
...
void Weapon_Blaster (edict_t *ent)
{
	...
	static int	fire_frames[]	= {5, 0};
	//to make a weapon shoot faster: modify X (was 8, can only be 5-8) in Weapon_Generic(ent,4,X,52,55, pause_frames,,,...,)
...
void Weapon_HyperBlaster (edict_t *ent)
{
	...
	Weapon_Generic (ent, 5, 15, 49, 53, pause_frames, fire_frames, Weapon_HyperBlaster_Fire);// was 20
}
...
void Weapon_Chaingun (edict_t *ent)
{
	...
	Weapon_Generic (ent, 4, 10, 61, 64, pause_frames, fire_frames, Chaingun_Fire); // was 31
}
...
void Weapon_Shotgun (edict_t *ent)
{
	...
	Weapon_Generic (ent, 7, 9, 36, 39, pause_frames, fire_frames, weapon_shotgun_fire);// was 18
}
...
void Weapon_SuperShotgun (edict_t *ent)
{
	...
	Weapon_Generic (ent, 6, 8, 57, 61, pause_frames, fire_frames, weapon_supershotgun_fire); // was 17
}
...
void weapon_railgun_fire (edict_t *ent)
{
	...
	VectorSet(offset, 0, -14, ent->viewheight - 8);
	P_ProjectSource(ent->client, ent->s.origin, offset, forward, right, start);
	fire_rail_poison(ent, start, forward, damage, kick);

	VectorSet(offset, 0, 28, ent->viewheight - 8);
	P_ProjectSource(ent->client, ent->s.origin, offset, forward, right, start);
	fire_rail_poison(ent, start, forward, damage, kick);
...
void Weapon_Railgun (edict_t *ent)
{
	...
	Weapon_Generic (ent, 3, 4, 56, 61, pause_frames, fire_frames, weapon_railgun_fire); // was 18
}
...
void Weapon_BFG (edict_t *ent)
{
	...
	Weapon_Generic (ent, 8, 22, 55, 58, pause_frames, fire_frames, weapon_bfg_fire);// was 32
}

/* FOR THE TRANQUILIZER*/
void Dart_Fire(edict_t *ent)
{
	int	i;
	vec3_t		start;
	vec3_t		forward, right;
