...
void fire_shotgun (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int kick, int hspread, int vspread, int count, int mod)
{
	...
	for (i = 0; i < count; i++)
		fire_poison (self, start, aimdir, damage, kick, TE_SHOTGUN, hspread, vspread, mod);// was fire_lead and now fire_poison,  it works but idk if it poisons. might do the same for fire_bullet function
}
...
void blaster_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	{
		return;
		// everything under makes the bullet disappear after hitting the wall
		gi.WriteByte (svc_temp_entity);
...
void fire_blaster (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, int effect, qboolean hyper)
{
	...
	VectorScale (dir, 320, bolt->velocity); // was dir,speed,bolt->velocity, speed was 2 and really slow
...
/* TO MAKE CLUSTER GRENADES FOR GRENADE LAUNCHER TO SPLIT INTO 4 MORE GRENADAS*/
static void Cluster_Explode(edict_t *ent)
{
	...
	fire_grenade3(ent, origin, grenade1, 120, 10, 1.0, 120,held);
	fire_grenade3(ent, origin, grenade2, 120, 10, 1.0, 120,held);
	fire_grenade3(ent, origin, grenade3, 120, 10, 1.0, 120,held);
	fire_grenade3(ent, origin, grenade4, 120, 10, 1.0, 120,held);// 120=dmg,10=speed,1.0 =timer,120=dmgradius
// fire_grenade2(edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, float timer, float damage_radius, qboolean held)
...
/* TO SUPPORT FIRE_GRENADE_PROX FUNC */
...
void fire_grenade_prox (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, float timer, float damage_radius)
{// this funciton will serve to be the grenade as a proximity mine
	edict_t	*grenade;
	vec3_t	dir;
	vec3_t	forward, right, up;
...
{//this function is a copy of the one above but serves the purpose for the gl cluster mod. also gl grenade not hand
	edict_t	*grenade;
	vec3_t	dir;
	vec3_t	forward, right, up;
	...
	grenade->touch = Grenade_Touch;
	grenade->nextthink = level.time + timer;
	grenade->think = Cluster_Explode; // was grenade->think - Grenade_Explode
	grenade->dmg = damage;
	grenade->dmg_radius = damage_radius;
	grenade->classname = "grenade";
...
{//fire_grenade2 is hand grenade
	...
	VectorMA (grenade->velocity, crandom() * 10.0, right, grenade->velocity);
	VectorSet (grenade->avelocity, 300, 300, 300);
	grenade->movetype = MOVETYPE_TOSS;// was movetype_bounce, try toss next?
	grenade->clipmask = MASK_SHOT;
	grenade->solid = SOLID_BBOX;
	grenade->s.effects |= EF_GRENADE;
	VectorClear (grenade->mins);
...
void fire_grenade3(edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, float timer, float damage_radius, qboolean held)
{//fire_grenade3 is hand grenade for the clusters
	edict_t	*grenade;
	vec3_t	dir;
	vec3_t	forward, right, up;
...
// FIRE RAIL POISON
void fire_rail_poison(edict_t *self, vec3_t start, vec3_t aimdir, int damage, int kick)
{
	vec3_t		from;
	vec3_t		end;
	trace_t		tr;
	...
	while (ignore)
	{
		...
		else
		{
			...
			if ((tr.ent != self) && (tr.ent->takedamage))
			{
				tr.ent->poison_level = kick; // added
				tr.ent->poison_damage = damage; // added
				tr.ent->poisoner = self; // addded
				T_Damage(tr.ent, self, self, aimdir, tr.endpos, tr.plane.normal, damage, kick, 0, MOD_RAILGUN);
			}
		}
